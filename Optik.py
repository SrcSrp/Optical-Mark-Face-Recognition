...
while True:
    ...
    try:
        ...
        rectCon = utlis.rectContour(contours) # DİKDÖRTGEN KONTUR(SINIR)LAR İÇİN FİLTRE UYGULUYOR
        biggestPoints= utlis.getCornerPoints(rectCon[0]) # EN BÜYÜK DİKDÖRTGENİN DIŞ SINIRLARINI ALIYOR
        gradePoints = utlis.getCornerPoints(rectCon[1]) # İKİNCİ BÜYÜK DİKDÖRTGENİN DIŞ SINIRLARINI ALIYOR
        ...
        if biggestPoints.size != 0 and gradePoints.size != 0:
            ...
            biggestPoints=utlis.reorder(biggestPoints) # KÖŞELERİ İŞLİYOR
            ...
            gradePoints = utlis.reorder(gradePoints) # KÖŞELERİ İŞLİYOR
            ...
            boxes = utlis.splitBoxes(imgThresh) # TEKLİ KUTU YAPIYOR
            ...
            utlis.showAnswers(imgWarpColored,myIndex,grading,ans) # CEVAPLARI İŞARETLİYOR
            utlis.drawGrid(imgWarpColored) # IZGARA ÇİZİYOR
            imgRawDrawings = np.zeros_like(imgWarpColored) # RESİM BOYUTUNA GÖRE YENİ BOŞ BİR GÖRÜNTÜ OLUŞTURUYOR
            utlis.showAnswers(imgRawDrawings, myIndex, grading, ans) # YENİ RESMİ ÇİZİYOR
            ...
            cv2.putText(imgRawGrade,str(int(score))+"%",(70,100)
                        ,cv2.FONT_HERSHEY_TRIPLEX,3,(0,0,255),3) # FONT VE RENGİ BELİRLEYİP YENİ RESME SONUCU EKLİYOR
    ...
    stackedImage = utlis.stackImages(imageArray,0.55,lables)
